...
For example, in a recent project, I documented why I chose SQLite over PostgreSQL (simplicity of deployment and adequate performance for the expected scale), which saved me from second-guessing that decision months later.
...
Comments that merely describe what code does waste space and can drift out of sync with the code. Instead, I focus my comments on:
...
Some of the most helpful documentation is code that shows how other code should be used. I create example files that:
...
In a Python module I built for parsing specialized config files, I included an `examples` directory with sample input files and scripts showing how to process them. When I came back a year later, these examples got me productive again within minutes.
...
I once spent a day trying to debug why a previously working project suddenly failed, only to realize I had upgraded Node.js in the meantime. A simple note about version requirements would have saved hours.

## Schedule Documentation Reviews

Documentation rots faster than code. A function with a slightly misleading comment is worse than a function with no comment at all. I schedule recurring calendar reminders to:
...
I use a simple text file with date headers and bullet points about:
- What I accomplished
...
When working on open source or projects with potential longevity, I take documentation a step further by considering:
...
To make good documentation habits stick, I've created lightweight templates for common documentation needs:
...
These templates provide just enough structure to ensure I cover important information without becoming burdensome.
